...
| `MAX_CHANNEL` | `channel_animenaruto` или числовой `chat_id` |
...
Публичный MAX-канал автоматически разрешается через MAX API по значению
`channel_animenaruto`; вручную искать числовой `chat_id` не требуется.
