...
провайдера: Base URL, модель и API-ключ. Провайдеры чередуются между постами;
при таймауте или ошибке бот автоматически пробует второй и временно ставит
неисправный провайдер на паузу. Кнопка
...
виден в очереди до публикации и может быть перегенерирован кнопкой. Автоматическая
генерация ждёт 90 секунд после добавления поста и делает паузу 20 секунд между
запросами; ручная генерация запускается сразу. Новости и
...
Ошибки провайдеров, переключения и успешные генерации записываются отдельными
событиями `AI · ...` на странице «События». Эти интервалы можно изменить там же,
